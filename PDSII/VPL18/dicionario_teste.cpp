...
TEST_CASE("Testa Dicionario::valor(chave)") {
  ...
  d.Inserir("c1", "v1");
  d.Inserir("c4", "v4");
  CHECK_EQ(d.valor("c1"), "v1");
  CHECK_EQ(d.valor("c2"), "v2");
  CHECK_EQ(d.valor("c3"), "v3");
  CHECK_EQ(d.valor("c4"), "v4");
}
...
TEST_CASE("Testa Dicionario::Alterar(chave, valor)") {
  ...
  d.Alterar("c2", "x2");
  CHECK(d.pertence("c2"));
  CHECK_EQ(d.valor("c2"), "x2");
  CHECK(d.tamanho() == 4);

  d.Alterar("c3", "x3");
  CHECK(d.pertence("c3"));
  CHECK_EQ(d.valor("c3"), "x3");
  CHECK(d.tamanho() == 4);

  d.Alterar("c1", "x1");
  CHECK(d.pertence("c1"));
  CHECK_EQ(d.valor("c1"), "x1");
  CHECK(d.tamanho() == 4);

  CHECK(d.pertence("c4"));
  CHECK_FALSE(d.valor("c4") == "x4");
  CHECK(d.tamanho() == 4);
}
